...
Search thoroughly across multiple sources. Include specific numbers.`;
      ...
      const p1=`You are an elite ${gameName} theorycrafter with deep knowledge of weapons, stats, item locations, and optimal progression routes. Generate the FIRST HALF of an OP build (metadata + 3 early phases).${customGameGuard}
CRITICAL OUTPUT FORMAT: Your ENTIRE response must be a single JSON object. No preamble, no commentary, no explanation before or after. No "Here's the build:" or "Based on my research". Start your response with { and end with }. Your output is parsed by code that will fail if there's any non-JSON text.${useCustomGame?`\n\nIMPORTANT: ${gameName} is not yet in the codex. You MUST determine its stat system via web search, then include a "game_meta" field at the top of your JSON. ALSO detect whether this game has New Game Plus and set has_ng_plus accordingly (e.g. Deepwoken has NO NG+, so has_ng_plus: false).`:""}
...
COMPLETE REFERENCE EXAMPLE (match this depth and specificity — items are from Lords of the Fallen, NOT ${gameName}; use ${gameName}-specific items only):${useCustomGame?`\n⚠ CUSTOM GAME: The items below are Lords of the Fallen examples. Do NOT use them. Only use real ${gameName} items.`:""}
...
Generate exactly 3 phases using this game's actual progression system. Phase 1 (Early Game), Phase 2 (Core Weapon/Ability — acquires the build's signature item from a specific world location), Phase 3 (Key Accessories/Gear — build-defining equipment with exact locations). Use the game's level term (e.g. "Lv", "Power", "Soul Level") in the "range" field — do NOT default to "Lv" if the game uses a different term.
...
${userConstraints}`;
      ...
      const p2Phase7Schema=hasNGPlus
        ...
        :`Phase 7 (Max Build / Final Optimization — NO NG+ in this game): {"name":"Max Build","range":"Endgame Max","stats":{${resolvedStatObjStr}},"sn":"Final stat targets at cap","weapons":[...],"armor":[...],"acc":[...],"spells":[...],"dmg":{"ps":"~N peak","sp":"...","bs":"...","n":"Final synergies and optimization notes. No NG+ cycles needed."}}`;
      const p2GenerateLine=hasNGPlus
        ?`Generate: Phase 4 (Unlock Key Abilities, ${levelTerm} 25-40), Phase 5 (Mid-to-Late / Core Synergies, ${levelTerm} 35-55), Phase 6 (Endgame, ${levelTerm} 55+), Phase 7 (NG+, with ngCycles) ${p2RangeEx}`
        :`Generate: Phase 4 (Unlock Key Abilities, ${levelTerm} 25-40), Phase 5 (Build Synergies, ${levelTerm} 35-55), Phase 6 (Endgame, ${levelTerm} 55+), Phase 7 (Max Build / Final Optimization — no NG+ exists in this game) ${p2RangeEx}`;
      const p2NGPlusRules=hasNGPlus
        ?`Stats approach/hit soft caps in endgame, hard caps (${statMax}) in NG+7. Each NG+ cycle adds ~5-10 levels per stat.`
        :`Stats reach their caps in Phase 6-7. There is NO NG+ in this game — Phase 7 covers final build optimization and peak performance.`;
      // Reference example with explicit game-isolation note for custom games
      const refExampleNote=useCustomGame?`\nNOTE ON EXAMPLE BELOW: This example uses Lords of the Fallen items. For ${gameName} builds, use ONLY ${gameName}-specific items — do NOT copy any item names from this example.\n`:"";

      const p2=`You are continuing a "${step1.label}" (${step1.sub}) build guide for ${gameName}. Phases 1-3 have already been written. Generate the LATE progression phases 4-7.${customGameGuard}
...
SCHEMA: {"ph":[<phase4>,<phase5>,<phase6>,<phase7>]}

Phases 4-6 schema: {"name":"Phase Name","range":"${levelTerm} X-Y","stats":{${resolvedStatObjStr}},"sn":"stat note","weapons":[{"n":"...","ap":"~N","sc":"scaling grades","st":"...","eq":true,"d":"...","loc":"...","up":"...","tip":"..."}],"armor":[{"n":"...","wt":"~N","eq":true,"d":"...","loc":"...","up":"N/A","tip":"..."}],"acc":[{"n":"...","ef":"precise effect with numbers","eq":true,"d":"...","loc":"...","up":"N/A","tip":"..."}],"spells":[{"n":"Spell","ef":"precise effect with numbers","dmg":"~N damage or N buildup/cast","eq":true,"d":"desc","loc":"where/how learned","up":"scaling stat","tip":"tip"}],"dmg":{"ps":"~N","sp":"...","bs":"...","n":"..."}}

ITEM FIELD REQUIREMENTS — every field must contain specific, non-placeholder data:
- "n": A REAL item name from ${gameName}. NEVER write generic placeholders like "2nd weapon", "another ring", "best armor", "based on loadout". Use real searchable item names.
- "d": WHY this item matters for THIS build (not a generic description).
- "loc": EXACT location — zone, NPC/merchant name, boss drop, chest location. If item was obtained in Phases 1-3, write "Obtained in Phase X — [zone]". NEVER write "Acquired", "Exploration", "Mid-game", "Various", or standalone "N/A".
...
- "ef" (accessories & spells): precise mechanic WITH numbers. NEVER write vague phrases like "improves status" or "boosts damage".
- "dmg" (spells only): actual damage or status buildup per cast/hit. Use real numbers.
${refExampleNote}
COMPLETE REFERENCE EXAMPLE (depth/format to match — items are from Lords of the Fallen, NOT ${gameName}):
{"name":"Unlock Spells","range":"Lv 25-40","stats":{"VIT":25,"END":24,"STR":32,"AGI":10,"RAD":18,"INF":12},"sn":"RAD to 15+ for Lacerating Weapon. Push STR past 30.","weapons":[{"n":"Bloody Glory (+5 to +7)","ap":"~700-850","sc":"A STR / B+ RAD","st":"360 Bleed w/ Lacerating Weapon","eq":true,"d":"Lacerating Weapon adds +60 Bleed on top of the base 300, pushing proc threshold down to 1-2 swings on bosses.","loc":"Obtained in Phase 2 — Pilgrim's Perch, Path of Devotion.","up":"Large Deralium Shards — farm from Holy Bulwark enemies or purchase from Gerlinde.","tip":"Cast Lacerating Weapon before every boss pull — the Bleed boost is that significant."}],"armor":[{"n":"Fitzroy's Set","wt":"71.7","eq":true,"d":"Highest poise-to-weight in early game.","loc":"Fitzroy's Gorge — chest at top of tower.","up":"N/A","tip":"Keep equip load below 70% for medium roll."}],"acc":[{"n":"Bloodbane Ring","ef":"Bleed when you inflict Poison","eq":true,"d":"Core engine — Poison proc triggers Bleed simultaneously.","loc":"Forsaken Fen — eastern swamp shore.","up":"N/A","tip":"Never unequip."}],"spells":[{"n":"Lacerating Weapon","ef":"+60 Bleed buildup per hit for 60s","dmg":"+60 Bleed/swing","eq":true,"d":"Elevates Bleed buildup from 300 to 360 per hit.","loc":"Radiance spell — purchase from Molhu at Vestige of Blind Agatha for ~4,500 vigor. Requires 15 RAD.","up":"Scales with RAD.","tip":"Cast first before Poison Weapon."}],"dmg":{"ps":"~700-850 + Pendant amp","sp":"Bleed in 1-2 swings","bs":"Very fast","n":"Pre-buffed, most enemies die in 3-4 hits."}}

${p2Phase7Schema}

${p2GenerateLine}

RULES: ${p2NGPlusRules} Use exact stat keys: ${resolvedStatKeysStr}. For any NEW items introduced in phases 4-6, verify the exact zone and landmark before writing "loc".${urlRef}${knowledgeBlock}${researchContext}
...
Main build: "${step1.label}" (${step1.sub}) - ${step1.playstyle}`;
  ...
  const handleWikiImport=async()=>{
    ...
    const htmlToText=(rawHtml,maxLen=56000)=>{
      ...
      // img alt text — Fextralife stat icons use alt="Physical", alt="Smite", etc.
      w=w.replace(/<img[^>]+alt=["']([^"']{2,100})["'][^>]*\/?>/gi,(_,alt)=>{
        const t=alt.trim();
        // Skip generic decorative alts (arrows, logos, etc.)
        if(/^(arrow|icon|logo|banner|bg|background|decoration|edit|pencil|search|menu|close|next|prev|left|right|up|down)$/i.test(t))return" ";
        return` [${t}] `;
      });
      w=w.replace(/<img[^>]+title=["']([^"']{2,100})["'][^>]*\/?>/gi,(_,t)=>` [${t.trim()}] `);
      w=w.replace(/<img[^>]*\/?>/gi," ");
      // title= on any element (tooltip text often holds the full stat name/value)
      w=w.replace(/\stitle=["']([^"']{3,100})["']/gi,(_,t)=>` [${t.trim()}]`);
      // data-value / data-stat / data-amount etc.
      w=w.replace(/\sdata-(?:value|stat|amount|bonus|defense|weight|damage|scaling)=["']([^"']{1,60})["']/gi,(_,v)=>` ${v}`);
      // ── 2. Headings → ## markers ───────────────────────────────────────────
      // CRITICAL: piece cards (Abbess Mitre, Abbess Garb, …) are introduced by
      // headings. Preserving them lets the AI know which stat table belongs to
      // which piece — without this, 4 pieces' numbers all look identical.
      w=w.replace(/<h[1-6][^>]*>([\s\S]*?)<\/h[1-6]>/gi,(_,inner)=>{
        const text=inner.replace(/<[^>]+>/g,"").replace(/\s+/g," ").trim();
        return text?`\n\n## ${text} ##\n\n`:" ";
      });
      // ── 3. Preserve table structure with readable separators ───────────────
      ...
      w=w.replace(/<\/t[hd]>/gi," · ");
      w=w.replace(/<br\s*\/?>/gi,"\n");
      // ── 4. Section-breaking divs ───────────────────────────────────────────
      // Fextralife renders piece cards as <div class="…"> blocks.
      // Add a blank line before each new div so pieces don't bleed together.
      w=w.replace(/<div[^>]+class=["'][^"']*(?:wiki_table|item[_-]?card|armor[_-]?card|piece[_-]?card|stat[_-]?block|infobox)[^"']*["'][^>]*>/gi,"\n\n");
      // ── 5. Nav / chrome removal ────────────────────────────────────────────
      w=w.replace(/<nav[\s\S]*?<\/nav>/gi," ");
      w=w.replace(/<header[\s\S]*?<\/header>/gi," ");
      w=w.replace(/<footer[\s\S]*?<\/footer>/gi," ");
      // ── 6. Isolate main wiki content ───────────────────────────────────────
      const patterns=[
      ...
      // ── 7. Strip remaining tags, decode entities, normalise whitespace ──────
      return w
        .replace(/<[^>]+>/g," ")
